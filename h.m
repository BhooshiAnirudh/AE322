...
dampingRatio = 0.517;              % Desired damping ratio
naturalFrequency = 4.4 / (dampingRatio * 0.5);  % Natural frequency

% Calculate PD gains
Kp = (-naturalFrequency^2 - 21.582) / 2;
Kd = -dampingRatio * naturalFrequency;

% Closed-loop system matrix (fixed concatenation)
A_closed = [0, 1; ...
            ((cartMass + pendulumMass) * gravity + Kp) / (pendulumLength * cartMass), ...
            Kd / (pendulumLength * cartMass)];

% Create closed-loop system
closedLoopSystem = ss(A_closed, B, C, D);
...
[linearResponse, time, linearStates] = initial(closedLoopSystem, initialState, timeSpan);
...
xlabel('Time (s)');
ylabel('\theta (rad)');
title('Linearized System with PD Control');
grid on;
...
xlabel('Time (s)');
ylabel('\theta (rad)');
title('Nonlinear System with PD Control');
grid on;
